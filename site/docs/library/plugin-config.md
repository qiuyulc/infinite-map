...
本文列出 `createDefaultEditorPluginsWithUI()` 的所有可配置项及其默认值。
...
| `includeOrigin` | `boolean` | `true` | 是否在计算世界范围时强制包含原点 (0,0) |
...
| `requireShift` | `boolean` | `false` | 是否需要按住 Shift 才能框选（`true` 时空白拖拽=平移） |
| `storeKey` | `string` | — | 自定义 store key |
| `selectionKey` | `string` | — | 自定义 selection key |
| `minDragPx` | `number` | — | 形成框选所需的最小拖动距离（px） |
...
| 参数 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `commandShortcuts` | `Record<string, string \| null>` | — | 以 commandId 为维度的快捷键覆盖。`null` 禁用该命令的快捷键 |

```ts
// 示例：禁用默认的 Mod+Z 撤销快捷键，改用 Mod+Shift+Backspace
...
如果不需要任何 HUD 组件，使用 `createDefaultEditorPlugins()`：
