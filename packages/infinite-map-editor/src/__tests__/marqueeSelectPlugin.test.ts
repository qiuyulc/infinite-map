...
describe('createMarqueeSelectPlugin', () => {
  it('requireShift=true: without shift it should not start marquee', () => {
    ...
    const res = plugin.handlers!.onPointerDown!(pe('down', { screen: { x: 500, y: 500 }, world: { x: 500, y: 500 } }), ctx);
  ...
  it('drag selects intersecting nodes; locked/hidden are excluded', () => {
    ...
    plugin.handlers!.onPointerDown!(
      pe('down', { screen: { x: -5, y: -5 }, world: { x: -5, y: -5 }, modifiers: { shift: true, alt: false, ctrl: false, meta: false } }),
      ctx
    );
    plugin.handlers!.onPointerMove!(pe('move', { screen: { x: 25, y: 25 }, world: { x: 25, y: 25 } }), ctx);
    plugin.handlers!.onPointerUp!(pe('up', { screen: { x: 25, y: 25 }, world: { x: 25, y: 25 } }), ctx);
  ...
  it('shift=true unions selection; click blank clears unless shift', () => {
    ...
    plugin.handlers!.onPointerDown!(pe('down', { screen: { x: -5, y: -5 }, world: { x: -5, y: -5 }, modifiers: { shift: true, alt: false, ctrl: false, meta: false } }), ctx);
    plugin.handlers!.onPointerMove!(pe('move', { screen: { x: 12, y: 12 }, world: { x: 12, y: 12 } }), ctx);
    plugin.handlers!.onPointerUp!(pe('up', { screen: { x: 12, y: 12 }, world: { x: 12, y: 12 } }), ctx);
    expect(new Set(store.get<string[]>(STORE_KEYS.selectionIds))).toEqual(new Set(['a', 'b']));

    // blank click (no shift) => clear
    plugin.handlers!.onPointerDown!(pe('down', { screen: { x: 500, y: 500 }, world: { x: 500, y: 500 } }), ctx);
    plugin.handlers!.onPointerUp!(pe('up', { screen: { x: 500, y: 500 }, world: { x: 500, y: 500 } }), ctx);
    expect(store.get<string[]>(STORE_KEYS.selectionIds)).toEqual([]);

    // blank click with shift => keep
    store.set(STORE_KEYS.selectionIds, ['a']);
    plugin.handlers!.onPointerDown!(pe('down', { screen: { x: 500, y: 500 }, world: { x: 500, y: 500 }, modifiers: { shift: true, alt: false, ctrl: false, meta: false } }), ctx);
    plugin.handlers!.onPointerUp!(pe('up', { screen: { x: 500, y: 500 }, world: { x: 500, y: 500 }, modifiers: { shift: true, alt: false, ctrl: false, meta: false } }), ctx);
