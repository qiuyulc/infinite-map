...
export function createResizePlugin(opts: ResizePluginOptions = {}): InfiniteMapPlugin {
  ...
  const move = (e: MapPointerEvent, ctx: MapContext) => {
    ...
    if (cfg?.enabled) {
      ...
      setSnapGuides(
        ctx,
        vLines.length || hLines.length ? { v: vLines, h: hLines } : null,
        STORE_KEYS.snapGuides
      );
    } else {
    ...
    const patches: NodePatch[] = [
      { type: 'set', id: st.id, data: { x: next.x, y: next.y, width: next.w, height: next.h } },
    ];
  ...
  const end = (e: MapPointerEvent, ctx: MapContext) => {
    ...
    const patches: NodePatch[] = [
      { type: 'set', id: st.id, data: { x: last.x, y: last.y, width: last.w, height: last.h } },
    ];
